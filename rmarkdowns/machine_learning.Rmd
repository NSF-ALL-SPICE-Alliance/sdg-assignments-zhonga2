...
```{r}
k3 <- kmeans(sdr_data_imputed, centers = 3) # For Challenge #1, I changed the amount of clusters countries from 2 to 3, despite having a lower silhouette width
```
...
```{r}
fviz_cluster(k3, data = sdr_data_imputed) +
  theme_minimal() # Changed from k2 to k3
```
...
```{r}
country_clusters <- as.data.frame(k3$cluster) # Country_clusters dataframe will now categorize by 3 cluster countries
```
...
```{r}
# For challenge 2, I will train a random forest model to make predictions of "sustainable nitrogen management index" scores under sustainable development goal 2, "End Hunger"
rf_snmi <- randomForest(normalized_score_sdg2_snmi ~ .,
...
```{r}
rf_snmi # Only 35.02% of the variance in the target variable (normalized_score_sdg2_snmi) is explained by the predictor variables included in the model.
```
...
```{r}
importance_df <- as.data.frame(rf_snmi$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Sustainability Nitrogen Management Index",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
# Looking at the partial dependence plot of the influence of the most important influence, cereal yield (normalized_score_sdg2_crlyld), on sustainable nitrogen management index 
pdp::partial(rf_snmi, pred.var = "normalized_score_sdg2_crlyld", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_snmi, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg6_scarcew")) # Changed to rf_snmi and the prediction variance will be cereal yield (normalized_score_sdg2_crlyld) and water scarcity (normalized_score_sdg6_scarcew) -- the two most important predictor variables of rf_snmi
...
Changing the code chunk where it will identify 3 country clusters instead has changed the order where countries are placed.
- Cluster #1 (attributes are low dim1 and low dim2 values) holds predominately middle income countries, which varies in plenty of regions such as Latin America and Eastern Europe and represents the state the majority of human population is at (world). Countries include Vietnam and Egypt.
- Cluster #2 (attributes are low dim1 and high dim2 value) holds predominately high income and OCED countries, such as United Arab Emirates and the Netherlands. 
- Cluster #3 (attributes are high dim1 and high dim2 values) holds predominately low income countries and countries in the region of Sub-Saharan Africa or Oceania. Countries include Zimbabwe and Togo. 
...
- Rename the random forest model from rf_matmort to something that more accurately represents the model (base the name on the indicator you are predicting)
  I renamed the random forest model from rf_matmort to rf_snmi (snmi is short for "Sustainable Nitrogen Management Index" under the SDG 2, End Hunger)
