...
```{r}
# Get a subset of sdr_data of goal_8_score and goal_9_score
SDG_scores <- 
  sdr_data %>%
  select(goal_8_score, goal_9_score)
...
```{r}
# Selects distinct values in regions_used_for_the_dsr column in sdr_data
sdr_data %>%
  select(regions_used_for_the_sdr) %>%
  group_by(regions_used_for_the_sdr) %>%
  distinct(regions_used_for_the_sdr)

# Create a new dataframe called new_region where data is filtered by the Sub-Saharan Africa region
new_region <-
  sdr_data %>%
  filter(regions_used_for_the_sdr == "Sub-Saharan Africa")

# Prints new_region on solve
new_region
```
